...
var MessagesZhCN = map[string]string{
	...
	"validation.model_mapping_only_for_aggregate":            "模型映射仅支持聚合分组",
	"validation.model_mapping_requires_sub_groups":           "请先配置子分组后再设置模型映射",
	"validation.model_mapping_alias_required":                "模型别名不能为空",
	"validation.model_mapping_duplicate_alias":               "模型别名 {{.model}} 重复",
	"validation.model_mapping_targets_required":              "模型别名 {{.model}} 必须至少指定一个目标子分组",
	"validation.model_mapping_duplicate_target":              "模型别名 {{.model}} 存在重复的子分组+实际模型配置",
	"validation.model_mapping_invalid_sub_group":             "模型别名 {{.model}} 引用了不存在的子分组（ID {{.sub_group_id}}）",
	"validation.model_mapping_invalid_weight":                "模型别名 {{.model}} 配置的子分组 {{.sub_group_id}} 权重无效",
	"validation.model_mapping_target_model_required":         "模型别名 {{.model}} 需要指定实际使用的模型名称",
