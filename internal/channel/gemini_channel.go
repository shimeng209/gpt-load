...
func (ch *GeminiChannel) ValidateKeyWithModel(ctx context.Context, apiKey *models.APIKey, group *models.Group, model string) (bool, error) {
...
func (ch *GeminiChannel) validateWithSpecificModel(ctx context.Context, apiKey *models.APIKey, group *models.Group, model string) (bool, error) {
	upstreamURL := ch.getUpstreamURL()
	if upstreamURL == nil {
		return false, fmt.Errorf("no upstream URL configured for channel %s", ch.Name)
	}

	// Gemini requires the model in the URL path and API key as query parameter
	reqURL, err := url.JoinPath(upstreamURL.String(), "v1beta", "models", model+":generateContent")
	if err != nil {
		return false, fmt.Errorf("failed to create gemini validation path: %w", err)
	}
	reqURL += "?key=" + apiKey.KeyValue
...
func (ch *GeminiChannel) validateWithDefaultModel(ctx context.Context, apiKey *models.APIKey, group *models.Group) (bool, error) {
	return ch.ValidateKey(ctx, apiKey, group)
}
